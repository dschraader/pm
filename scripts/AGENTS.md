...
- `ai-chat-smoke.sh` - logs in and sends a single chat turn ("What cards are currently in the Backlog column?") to `POST /api/ai/chat`, printing the full JSON `{reply, appliedMutations, board}` response. Same env-var requirement as `ai-smoke.sh`.
