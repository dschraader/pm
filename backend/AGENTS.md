...
- `app/db.py` - SQLite layer: `DB_PATH` (env-driven, defaults to `/app/data/pm.db`), `connect()` (opens with `check_same_thread=False` because FastAPI's sync-route threadpool may dispatch yield vs. cleanup onto different threads; sets `foreign_keys=ON`), `get_db()` FastAPI dependency (commits on success, rolls back on exception), `init_db()` runs DDL with `IF NOT EXISTS` and seeds the default user/board on a fresh DB. See `docs/DATABASE.md` for the schema.
